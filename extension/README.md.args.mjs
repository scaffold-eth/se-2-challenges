...
<details markdown='1'><summary>❓ Why do on-chain SVG NFTs matter?</summary>
Most NFTs store their images on IPFS or centralized servers -- if those go down, your NFT becomes a broken link. On-chain SVG NFTs solve this by embedding the artwork directly in the smart contract. The image can be reconstructed forever from the blockchain alone, making it truly permanent and decentralized.

Because the SVG is generated by code, it can also be dynamic -- changing based on on-chain state, composable with other contracts, and verifiable by anyone.
</details>

---

🖼️ **"Why put art on the blockchain?"** Because most NFT art isn't actually on the blockchain -- it's a link to a server that could go down tomorrow. On-chain SVGs are permanent by construction.

🎨 [Art Blocks](https://www.artblocks.io/) pioneered fully on-chain generative art. Projects like Chromie Squiggles and Fidenza store their generation algorithms directly on Ethereum, producing unique outputs at mint time. Some pieces have sold for hundreds of thousands of dollars -- and the code that creates them will outlive any server.

⌐◨-◨ [Nouns](https://nouns.wtf/) generates one new pixel-art character every 24 hours, entirely on-chain. The SVG rendering lives in the contract -- no IPFS, no servers. The artwork exists as long as Ethereum exists, and it's been forked hundreds of times by other communities building their own on-chain identities.

🧬 **Dynamic NFTs** are the next frontier. Because the SVG is code, not a static file, it can evolve based on on-chain events -- an NFT that changes with the owner's history, governance participation, or time held. On-chain rendering makes this possible without any centralized backend.

🔓 On-chain SVGs are **composable** -- your Loogies can be rendered inside other NFTs, used as avatars in onchain games, or combined with accessories from separate contracts. The rendering is just a function call.

🚀 Ready to create permanent, generative art? Let's go!
