---
name: skip
description: Skip the current coding task. The AI will write the solution code, explain it, and move to the next checkpoint. Use this if you're stuck and want to move forward.
---
...
Use the **progress-tracker** subagent:
```
Mark checkpoint "[checkpoint-id]" as completed with method "skipped". Set next checkpoint "[next-id]" to in_progress.
```

### Step 7: Continue
- If more checkpoints remain, present the next checkpoint's context (follow the teaching flow from `.ai/instructions/start-content.md`)
...
- **Use the progress-tracker subagent** for all progress file operations
- **Read `.ai/instructions/start-content.md`** for the full teaching flow when continuing to the next checkpoint
